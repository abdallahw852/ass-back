...
import { ApproveReturnHandler } from './approve-return.handler';
import {
...
describe('ApproveReturnHandler', () => {
  it('throws ReturnRequestNotFoundException when the return request does not exist', async () => {
    ...
    const handler = new ApproveReturnHandler(repo as never);

    await expect(
      handler.execute(new ApproveReturnCommand('missing-id', 1, 99)),
  ...
  it('throws ReturnRequestAccessDeniedException when the return belongs to another supplier', async () => {
    ...
    await expect(
      handler.execute(new ApproveReturnCommand('return-public-id', 1, 99)),
  ...
  it('throws InvalidReturnStatusTransitionException when the return is not pending', async () => {
    const repo = new FakeReturnRequestRepo(buildEntity({ status: 'approved' }));
    const handler = new ApproveReturnHandler(repo as never);
  ...
  it('approves a pending return request and returns the updated read model', async () => {
    const repo = new FakeReturnRequestRepo(buildEntity());
    const handler = new ApproveReturnHandler(repo as never);

    const result = await handler.execute(
      new ApproveReturnCommand('return-public-id', 1, 99),
    );
